...
export class JsPlumbService {
  ...
  public init(): void {
    ...
    this._instance = jsPlumb.getInstance({
      Connector: ['StateMachine', {cornerRadius: 5}],
      Container: 'canvas',
  ...
  public destroyModel(model: Model) {
    ...
    const connections_to_delete = this._instance.getConnections({
      source: model.getElementH2Id()
    });

    for (let i = 0; i < connections_to_delete.length; i++) {
      const schema_to_delete = this.dataService.data.getSchemaByElementId(connections_to_delete[i].targetId);
      if (schema_to_delete) {
  ...
  public initSchema(schema: Schema) {
    ...
    if (schema.belongsto) {
      ...
      const source_id = this.dataService.data.getModelByName(schema.belongsto).getElementH2Id();
      const target_id = schema.getElementId();
      const option = {
        source: source_id,
        target: target_id
      };
