...
export class DataService {
  ...
  public addSchema(schema: Schema): void {
    ...
    const obj_model = this.data.getModelById(schema.parent_id);

    // confirm changing schema id for relation or not change
    let f_schema_id_for_relation = false;
  ...
  public moveSchema(schema: Schema, dir: number): void {
    ...
    const len = obj_model.schemas.length;
    for (let i = 0; i < len; i++) {
      if (obj_model.schemas[i].id === schema.id) {

        // tslint:disable-next-line:no-shadowed-variable
        const filterd_schemas = obj_model.schemas.filter((v, i) => v.id !== schema.id);
  ...
  public editSchemaToLaravelUserModel(): void {
    ...
    const model_user = this.data.getModelByName('user');
